...
namespace CitiesAutoCompete.Tests.Controllers
{
	...
	{
		private SuggestionsController _suggestions;
		private Mock<ICitySearchService> _citySearchServiceMock;
		private Mock<ILogger<SuggestionsController>> _loggerMock;
		...
		public void Setup()
		{
			...
			_loggerMock = new Mock<ILogger<SuggestionsController>>();
			_suggestions = new SuggestionsController(AutoMapper.Mapper.Instance, _citySearchServiceMock.Object, _loggerMock.Object);
		}
		...
		public async Task GivenInternalErrorShouldReturnError500()
		{
			...
			var p = new QueryParameters { q = "search" };
		...
		public async Task QueryParametersShouldBePassedToCitySearchService()
		{
			...
			var p = new QueryParameters { q = "search", Longitude = 1, Latitude = 2 };
			var response = await _suggestions.GetSuggestions(p);
